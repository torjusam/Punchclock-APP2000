...
export function formatInterval(interval: Interval) {
    if (!interval) return '-';
    if (interval.hours) {
        return `${interval.hours.toString().padStart(2, '0')}t ${interval.minutes ? interval.minutes.toString().padStart(2, '0') + 'm' : '00m'}`;
    } else if (interval.minutes) {
        return `${interval.minutes.toString().padStart(2, '0')}m ${interval.seconds ? interval.seconds.toString().padStart(2, '0') + 's' : '00s'}`;
    } else {
        return `00m ${interval.seconds ? interval.seconds.toString().padStart(2, '0') + 's' : '00s'}`;
    }
}