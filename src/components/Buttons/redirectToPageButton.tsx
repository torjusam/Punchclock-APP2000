...
const RedirectToPageButton: React.FC<RedirectToPageButtonProps> = ({employee}) => {
    ...
    if (!employee) {
        return (
            <button className={`${styles.buttonBase} ${styles.disabledButton}`}>
                <img className={`${styles.icon} ${styles.iconDisabled}`} style={{maxWidth: '3em', marginRight: '0.7em'}}
                     src="/stockAvatar.png" alt="avatar"/>
                Personlig side
            </button>
        )
    }
    ...
    return (
        <button onClick={goToPersonalPage} className={`${styles.buttonBase} ${styles.activeButton}`}>
            <img className={`${styles.icon} ${styles.iconActive}`} style={{maxWidth: '3em', marginRight: '0.7em'}}
                 src="/stockAvatar.png" alt="avatar"/>
            Personlig side
        </button>
    );
